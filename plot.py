...
for r in plot_regions:
    case_data.append(c.region_series(r))
for r in plot_states:
    case_data.append(c.state_series(r))
...
tmin_ts=float(tmin.timestamp())
tmax_ts=0
for n in range(0,len(case_data)):
    s=case_data[n]
    tsmax=s.dates[-1]+lookahead+86400.0
...
for n in range(0,len(case_data)):
    c=case_data[n]
    #s=active_data[n]
    max_deaths=c.deaths[-1]
    if max_deaths is None:
        max_deaths=c.deaths[-2]
    max_cases=c.cases[-1]
    if max_cases is None:
        max_cases=c.cases[-2]
    drate=100.0*(max_deaths/max_cases)
    t=[]
    for tval in c.dates:
        t.append(dt.datetime.fromtimestamp(int(tval)))
    ln,=ax1.semilogy(t,(c.cases-c.recovered),'-',color=plot_colours[c.location.state])
    ln.set_label(f'{c.location.state} ({max_cases:.0f}, {drate:.1f}%)')
    lines.append(ln)
    if c.location.state in fit_regions:
        # Generate look-ahead data based on exponenetial best fit
        if c.location.state=='Western Australia':
            idx=np.argwhere(c.cases > 10.0)
        else:
            idx=np.argwhere(c.cases > 100.0)
        t_fit=c.dates[idx[0][0]:]-tmin_ts
        y_fit=np.log(c.cases[idx[0][0]:])
        curve_fit=np.polyfit(t_fit,y_fit,1)
        t_fit=np.arange(c.dates[0],tmax_ts,86400.0)-tmin_ts # Generate t axis with look-ahead 10 days
        ...
        fit_ln,=ax1.semilogy(t,y_fit,':',color=plot_colours[c.location.state])
...
for n in range(0,len(active_data)):
    s=active_data[n]
    if c.location.state not in beds.keys():continue
    t=[]
    for tval in c.dates:
        t.append(dt.datetime.fromtimestamp(int(tval)))
    ln,=ax2.plot(t,100.0*s.y/beds[c.location.state],'-',color=plot_colours[c.location.state])
    ln.set_label(f'{c.location.state}')
    lines.append(ln)
    n+=1
n=0
"""
...
for n in range(0,len(drate_data)):
    ...
    for tval in c.dates:
        t.append(dt.datetime.fromtimestamp(int(tval)))
    ln,=ax2.plot(t,s.y,':',color=plot_colours[c.location.state])
    ln.set_label(f'{c.location.state}')
    n+=1
"""
"""
